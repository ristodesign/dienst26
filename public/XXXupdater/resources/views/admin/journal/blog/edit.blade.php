...
@section('content')
  <div class="page-header">
    ...
    <ul class="breadcrumbs">
      ...
      <li class="nav-item">
        <a href="{{ route('admin.blog_management.blogs', ['language' => $currentLang->code]) }}">{{ __('Posts') }}</a>
      </li>
      <li class="separator">
        <i class="flaticon-right-arrow"></i>
      </li>
      <li class="nav-item">
        @php
          $blogContent = $blog->information->where('language_id', $currentLang->id)->first();
        @endphp
        <a href="#">
          @if ($blogContent)
            {{ strlen($blogContent->title) > 50 ? mb_substr($blogContent->title, 0, 50, 'UTF-8') . '...' : $blogContent->title }}
          @else
            {{ '-' }}
          @endif
        </a>
