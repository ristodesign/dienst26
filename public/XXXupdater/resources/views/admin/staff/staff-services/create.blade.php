  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="ajaxForm" class="modal-form create" action="{{ route('admin.staff_service_assign.store') }}"
          method="post">
          <input type="hidden" name="staff_id" value="{{ request()->id }}">
          <input type="hidden" id="language" name="language" value="{{ request()->language }}">
          <input type="hidden" id="vendor_id" name="vendor_id" value="{{ request()->vendor_id }}">
          @csrf

          <div class="form-group">
            <label for="">{{ __('Services Title') . '*' }}</label>
