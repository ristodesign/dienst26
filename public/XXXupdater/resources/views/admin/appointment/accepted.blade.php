        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <div class="row">
                        <div class="col-lg-10">
                            <form id="searchForm" action="{{ route('admin.accepted_appointment') }}" method="GET">
                                <div class="row">
                                    <div class="col-lg-3">
                                    ...
                                    <div class="col-lg-3">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control h-42 select2" name="refund"
                                                onchange="document.getElementById('searchForm').submit()">
                                                <option value=""
                                                    {{ empty(request()->input('refund')) ? 'selected' : '' }}>
                                                    {{ __('All') }}
                                                </option>
                                                <option value="pending"
                                                    {{ request()->input('refund') == 'pending' ? 'selected' : '' }}>
                                                    {{ __('Pending') }}
                                                </option>
                                                <option value="refunded"
                                                    {{ request()->input('refund') == 'refunded' ? 'selected' : '' }}>
                                                    {{ __('Refunded') }}
                                                </option>
                                            </select>
                                        </div>
                                                    ...
                                                    <td>
                                                        ...
                                                        @else
                                                            @if ($item->payment_status == 'pending')
                                                                <form id="paymentStatusForm-{{ $item->id }}"
                                                    ...
                                                    <td>
                                                        @if (!empty($item->service->id))
                                                            @if ($item->order_status == 'accepted')
                                                                <h2 class="d-inline-block"><span
                                                                        class="badge badge-success">{{ __(ucfirst($item->order_status)) }}</span>
                                                                </h2>
                                                            @endif
                                                        @else
                                                            {{ '-' }}
                                                    ...
                                                    <td>
                                                        @if ($item->staff_id == null)
                                                            <a href="javascript::void(0)"
                                                                class="btn btn-sm btn-primary editBtn" data-toggle="modal"
                                                                data-target="#editModal_{{ $item->id }}"
                                                                data-appointment_id="{{ $item->id }}">{{ __('Assign') }}</a>
                                                        @else
                                                            @php
                                                                $staffContent = App\Models\Staff\StaffContent::where(
                                                                    'staff_id',
                                                                    $item->staff_id,
                                                                )
                                                        ...
                                                        @if ($item->refund != 'refunded')
                                                            <form id="refundStatus-{{ $item->id }}"
                                                                ...
                                                                <select
                                                                    ...
                                                                    <option value="pending"
                                                                        {{ $item->refund == 'pending' ? 'selected' : '' }}>
                                                                        {{ __('Pending') }}
                                                                    </option>
                                                                    <option value="refunded"
                                                                        {{ $item->refund == 'refunded' ? 'selected' : '' }}>
                                                                        {{ __('Refund') }}
                                                                    </option>
                                                                </select>
                                                            </form>
