...
function np_status_log($m)
{
    @file_put_contents(sys_get_temp_dir().'/nowpayments_invoice_status.log', '['.date('c')."] $m\n", FILE_APPEND);
}
...
try {
    ...
    if ($code >= 300) {
        np_status_log("api error ($code): ".json_encode($js));
        http_response_code($code);
        echo json_encode(['error' => 'NOWPayments query failed', 'raw' => $js]);
        exit;
    }
