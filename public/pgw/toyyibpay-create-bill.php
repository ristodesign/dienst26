...
function tp_log($m)
{
    @file_put_contents(sys_get_temp_dir().'/toyyib_create.log', '['.date('Y-m-d H:i:s')."] $m\n", FILE_APPEND);
}
...
try {
    ...
    if ($code >= 300) {
        ...
        echo json_encode(['error' => 'HTTP '.$code, 'raw' => $resBody]);
        exit;
    }
    ...
    if (! $billCode && is_string($resBody)) {
        if (preg_match('~"BillCode"\s*:\s*"([^"]+)"~i', $resBody, $m)) {
            $billCode = $m[1];
        }
    }
