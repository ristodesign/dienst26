...
class VendorDistanceController extends Controller
{
  ...
  public function index($address, $languageId, $sort = null)
  {
    $bs = Basic::select('google_map_api_key', 'google_map_status', 'google_map_radius')->first();
    ...
    $contents = VendorInfo::where('language_id', $languageId)
      ->select('vendor_id', 'address')
      ->get();

    foreach ($contents as $content) {
      $address2 = $content->address;
      $distance = null;
      ...
      if ($address2) {
        ...
        if ($distance !== null && $distance <= $bs->google_map_radius) {
          ...
          $vendorDistanceMap[$content->vendor_id] = $distance;
        }
      }
    }

    if ($sort == 'asc') {
      asort($vendorDistanceMap);  // Nearest to farthest
    } else if ($sort == 'desc') {
      arsort($vendorDistanceMap);  // Farthest to nearest
    }

    // Extract the sorted service IDs (keys) from the sorted map
    $sortedServiceIds = array_keys($vendorDistanceMap);
    return $sortedServiceIds;  // Return the sorted service IDs based on distance
  }
}
