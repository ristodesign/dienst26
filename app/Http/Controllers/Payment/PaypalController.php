...
class PaypalController extends Controller
{
  ...
  public function paymentProcess(Request $request, $_amount, $_title, $_success_url, $_cancel_url)
  {
    ...
    if ($bs->base_currency_text !== 'USD') {
      ...
      $convertedTotal = $_amount / $rate;
    }
    $paypalTotal = $bs->base_currency_text === 'USD' ? $_amount : $convertedTotal;
    $title = $_title;
    $price = round($paypalTotal, 2);
    ...
    try {
      ...
      return redirect($cancel_url)->with('error', $ex->getMessage());
    }
