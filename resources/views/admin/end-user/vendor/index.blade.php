...
@section('content')
  <div class="page-header">
    ...
    <ul class="breadcrumbs">
      ...
      <li class="nav-item">
        <a href="#">{{ __('Registered Vendors') }}</a>
      </li>
    </ul>
  </div>
                    ...
                    <tbody>
                      @foreach ($vendors as $vendor)
                        <tr>
                          ...
                          <td>
                            <form id="accountFeaturedForm-{{ $vendor->id }}" class="d-inline-block"
                              action="{{ route('admin.vendor_management.vendor.update_featured_status', ['id' => $vendor->id]) }}"
                              method="post">
                              @csrf
                              <select
                                class="form-control form-control-sm {{ $vendor->featured == 1 ? 'bg-success' : 'bg-danger' }}"
                                name="featured_status"
                                onchange="document.getElementById('accountFeaturedForm-{{ $vendor->id }}').submit()">
                                <option value="1" {{ $vendor->featured == 1 ? 'selected' : '' }}>
                                  {{ __('Active') }}
                                </option>
                                <option value="0" {{ $vendor->featured == 0 ? 'selected' : '' }}>
                                  {{ __('Deactive') }}
                                </option>
                              </select>
                            </form>
                          ...
                          <td>
                            <div class="dropdown">
                              ...
                              <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                <a href="{{ route('admin.vendor_management.vendor_details', ['id' => $vendor->id, 'language' => $currentLang->code]) }}"
                                  class="dropdown-item">
                                  {{ __('Details & Package') }}
                                </a>

                                <a href="{{ route('admin.edit_management.balance', ['id' => $vendor->id]) }}"
                                  class="dropdown-item">
