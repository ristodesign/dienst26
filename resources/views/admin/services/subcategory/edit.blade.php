  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form id="ajaxEditForm" class="modal-form" action="{{ route('admin.service_managment.subcategory.update') }}"
          method="post">
          @csrf
          <input type="hidden" id="in_id" name="id">
          <div class="form-group">
            <label>{{ __('Category') . '*' }}</label>
            <select name="category_id" class="form-control" id="in_category_id">
              <option selected disabled>{{ __('Select a Category') }}</option>
              @foreach ($categories as $categorie)
                <option value="{{ $categorie->id }}">{{ $categorie->name }}</option>
              @endforeach
            </select>
            <p id="editErr_category_id" class="mt-2 mb-0 text-danger em"></p>
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            <label>{{ __('Subcategory Status') . '*' }}</label>
            <select name="status" id="in_status" class="form-control">
              <option disabled>{{ __('Select Subcategory Status') }}</option>
          ...
          <div class="form-group">
            <label>{{ __('Subcategory Serial Number') . '*' }}</label>
            <input type="number" id="in_serial_number" class="form-control ltr" name="serial_number"
              placeholder="{{ __('Enter Subcategory Serial Number') }}">
