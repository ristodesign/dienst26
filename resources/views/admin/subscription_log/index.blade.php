            ...
            <div class="col-lg-12">
              ...
              @else
                <div class="table-responsive">
                      ...
                      @foreach ($memberships as $key => $membership)
                        <tr>
                          ...
                          <td>
                            ...
                            @if ($vendor)
                              <a
                                href="{{ route('admin.vendor_management.vendor_details', ['id' => $vendor->id, 'language' => $currentLang->code]) }}">{{ $vendor->username }}</a>
                            @endif
                          </td>
                          @php
                          ...
                          <td>
                            ...
                            @else
                              <form id="statusForm{{ $membership->id }}" class="d-inline-block"
                                action="{{ route('admin.payment-log.update', ['id' => $membership->id]) }}"
                                method="post">
                                @csrf
                  ...
                  @elseif ($membership->status == 0) bg-warning
                  @elseif ($membership->status == 2) bg-danger @endif"
                                  name="status"
                          ...
                          <div class="modal-dialog" role="document">
                            <div class="modal-content">
                              ...
                              <div class="modal-body">
                                ...
                                <p><strong>{{ __('Package Price') }}: </strong>
                                  {{ $membership->price }} {{ $membership->currency }}
                                </p>
                                <p><strong>{{ __('Method') }}: </strong> {{  __($membership->payment_method)  }}
                                </p>
                                ...
                                <p><strong>{{ __('Expire Date') }}: </strong>
                                  ...
                                  @else
                                    ...
                                    @else
                                      {{ __($membership->package->term) == 'lifetime' ? 'Lifetime' : \Illuminate\Support\Carbon::parse($membership->expire_date)->format('M-d-Y') }}
                                    @endif
                                  @endif
                                </p>
                                <p>
